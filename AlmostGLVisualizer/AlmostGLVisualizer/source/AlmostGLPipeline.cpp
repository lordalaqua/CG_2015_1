...
void AlmostGL::runVertexPipeline(const Model3D& model)
{
...
Vector3f AlmostGL::calculateIllumination(const Vector3f& vertex,
...
void AlmostGL::runRasterization()
{
    for (auto triangle : triangles)
    {
        if (mode == POINTS)
...
void AlmostGL::rasterTriangle(Triangle& triangle, FrameBuffer& buffer, 
    PolygonMode mode)
{
...
void AlmostGL::bresenham(const Vertex& start, const Vertex& end,
    FrameBuffer& buffer)
{
...
float AlmostGL::degreeToRadians(float angle)
{
    return angle*(3.14159265 / 180);
}

AlmostGL::Vertex AlmostGL::createInterpolated(const Vertex& start, 
...
void AlmostGL::writeToBuffer(const Vertex& v, FrameBuffer& buffer)
{
